...
public class MySQLDBConnection implements DBConnection {
	...
	public void unsetVisitedRestaurants(String userId, List<String> businessIds) {
		for (String bID : businessIds) {
			String sql = "DELETE FROM history WHERE `business_id` = \"" + bID + "\" AND `user_id` = \"" + userId + "\"";
	...
	/*private JSONArray sortByDistance(Set<JSONObject> set) {
		List<JSONObject> result = new LinkedList<>();
		try {
			while(!set.isEmpty()) {
				JSONObject restaurant = set.
				Double lat = (Double)restaurant.get("latitute");
			}
		} catch (Exception e) {
			System.out.println(e.getMessage());
		}
	...
	public Set<String> getCategories(String businessId) {
		try {
			if (conn != null) {
				// String sql = "SELECT categories FROM restaurants WHERE
	...
	public Set<String> getBusinessId(String category) {
		...
		try {
			if (conn != null) {
				// String sql = "SELECT business_id FROM restaurants WHERE
